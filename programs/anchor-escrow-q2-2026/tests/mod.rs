...
mod tests {
    ...
    fn test_make_and_take() {
        ...
        program.airdrop(&taker, 3_000_000_000).unwrap();
        msg!("Taker balance after airdrop: {}", program.get_balance(&taker).unwrap());

        // Taker's ATA for Mint A (to receive token A from the vault/maker)
        let taker_ata_a = CreateAssociatedTokenAccount::new(&mut program, &taker_keypair, &mint_a)
            .owner(&taker)
            .send()
            .unwrap();
        msg!("Taker ATA A: {}\n", taker_ata_a);
        ...
        // Maker's ATA for Mint B (to receive tokens from taker)
        ...
        msg!("Taker ATA B: {}\n", maker_ata_b);
         ...
         // Mint 2,000 tokens (with 6 decimal places) of Mint B to the taker's associated token account
        MintTo::new(&mut program, &payer, &mint_b, &taker_ata_b, 2000000000)
            .send()
            .unwrap();
        ...
        let take_ix = Instruction {
            ...
            accounts: anchor_escrow_q2_2026::accounts::Take {
                ...
                mint_a: mint_a,
                mint_b: mint_b,
                taker_ata_a: associated_token::get_associated_token_address(&taker, &mint_a),
            ...
            data: anchor_escrow_q2_2026::instruction::Take {

            }.data(),
        };

        let take_message = Message::new(&[take_ix], Some(&taker));

        let take_transaction = Transaction::new(&[&taker_keypair], take_message, program.latest_blockhash());
        ...
        // verify vault has been emptied and closed
        let vault_account_after_take = program.get_account(&vault);
        assert!(vault_account_after_take.is_none());
        
        
        //verify taker has received the tokens from the vault
        let taker_ata_a_account = program.get_account(&taker_ata_a);
        let taker_ata_a_data = spl_token::state::Account::unpack(&taker_ata_a_account.unwrap().data).unwrap();
        assert_eq!(taker_ata_a_data.amount, 10);
        assert_eq!(taker_ata_a_data.owner, taker);
        assert_eq!(taker_ata_a_data.mint, mint_a);

        //verify maker has received the tokens from the taker
        let maker_ata_b_account = program.get_account(&maker_ata_b);
        let maker_ata_b_data = spl_token::state::Account::unpack(&maker_ata_b_account.unwrap().data).unwrap();
        assert!(maker_ata_b_data.amount >= 10);
    ...
    fn test_make_and_refund() {
        ...
        let refund_transaction = Transaction::new(&[&payer], refund_message, program.latest_blockhash());
        ...
        // verify vault has been emptied and closed
        let vault_account_after_refund = program.get_account(&vault);
        assert!(vault_account_after_refund.is_none());

        //verify maker has received the tokens back from the vault
        let maker_ata_a_account = program.get_account(&maker_ata_a);
        let maker_ata_a_data = spl_token::state::Account::unpack(&maker_ata_a_account.unwrap().data).unwrap();
